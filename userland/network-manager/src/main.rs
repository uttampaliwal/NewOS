...
fn ipc_str(s: &str) -> IpcValue { IpcValue::String(s.to_string()) }
...
impl InterfaceController for LinuxInterfaceController {
    ...
    fn list_interfaces(&self) -> Vec<InterfaceInfo> {
        vec![InterfaceInfo { name: "eth0".into(), state: InterfaceState::Down, mac_address: "00:00:00:00:00:00".into() }]
    }
}
...
impl NetworkManager {
    fn new(config: NetworkConfig, controller: Box<dyn InterfaceController>) -> Self {
        ...
        Self { config, controller, interfaces }
    }
    ...
    fn bring_up(&mut self, name: &str) -> Result<Ipv4Addr, NetworkError> {
        ...
        if self.config.dhcp {
            ...
            self.set_iface_state(name, InterfaceState::Up {
                ip: lease.ip, gateway: lease.gateway, subnet_mask: lease.subnet_mask, dns: lease.dns_servers,
            });
            Ok(lease.ip)
            ...
            let dns: Vec<Ipv4Addr> = self.config.dns_servers.iter().filter_map(|s| s.parse().ok()).collect();
            self.controller.set_ip(name, ip, mask)?;
            self.controller.set_gateway(gw)?;
            if !dns.is_empty() { self.controller.set_dns(&dns)?; }
            self.controller.interface_up(name)?;
...
fn main() {
    ...
    let mut broker = match UnixStream::connect("/run/ipc.sock") {
        Ok(s) => s,
        Err(e) => { eprintln!("net-mgr: cannot connect to broker: {e}"); std::process::exit(1); }
    };

    let reg = IpcMessage::MethodCall {
        id: 1, interface: "org.turnix.Broker".into(), method: "Register".into(),
    ...
    if let Some(reply) = recv_msg(&mut broker) {
        match reply {
            ...
            _ => { eprintln!("net-mgr: broker registration failed"); std::process::exit(1); }
        }
    }
...
fn load_config(path: &str) -> NetworkConfig {
    if !Path::new(path).exists() { return NetworkConfig::default(); }
    match fs::read_to_string(path) {
        Ok(s) => NetworkConfig::from_toml(&s).unwrap_or_else(|e| {
            eprintln!("net-mgr: config error: {e}, using defaults"); NetworkConfig::default()
        }),
        Err(e) => { eprintln!("net-mgr: cannot read {path}: {e}"); NetworkConfig::default() }
    }
}
...
fn handle_ipc(manager: &mut NetworkManager, broker: &mut UnixStream, msg: IpcMessage) {
    let resp = match msg {
        IpcMessage::MethodCall { id, method, args, .. } => match method.as_str() {
            "Up" => {
                let name = args.first().and_then(|v| v.as_str()).unwrap_or("eth0");
                match manager.bring_up(name) {
                    Ok(ip) => IpcMessage::MethodReturn { id, result: Ok(ipc_str(&ip.to_string())) },
                    Err(e) => IpcMessage::MethodReturn { id, result: Err(IpcError::new(ERROR_INTERNAL, e.to_string())) },
                }
            }
            "Down" => {
                let name = args.first().and_then(|v| v.as_str()).unwrap_or("eth0");
                match manager.bring_down(name) {
                    Ok(()) => IpcMessage::MethodReturn { id, result: Ok(ipc_str("down")) },
                    Err(e) => IpcMessage::MethodReturn { id, result: Err(IpcError::new(ERROR_INTERNAL, e.to_string())) },
                }
            }
            "Status" => {
                let interfaces: Vec<IpcValue> = manager.interfaces.iter().map(|iface| {
                    ...
                    IpcValue::Map(vec![("name".into(), ipc_str(&iface.name)), ("state".into(), sv)])
                }).collect();
                IpcMessage::MethodReturn { id, result: Ok(IpcValue::Array(interfaces)) }
            }
            "Resolve" => {
                let hostname = args.first().and_then(|v| v.as_str()).unwrap_or("");
                if hostname.is_empty() {
                    IpcMessage::MethodReturn { id, result: Err(IpcError::new(ERROR_INVALID_ARGS, "missing hostname")) }
                } else {
                    resolve_dns(hostname, &manager.config, id)
                }
            }
            _ => IpcMessage::MethodReturn { id, result: Err(IpcError::new(ERROR_INTERNAL, format!("unknown method {method}"))) },
        },
        _ => return,
    };
...
fn resolve_dns(hostname: &str, config: &NetworkConfig, id: u64) -> IpcMessage {
    let dns_server: Ipv4Addr = config.dns_servers.first()
        .and_then(|s| s.parse().ok())
        .unwrap_or(Ipv4Addr::new(8, 8, 8, 8));

    use network_manager::{UdpDnsTransport, DnsResolver};
    let mut resolver = DnsResolver::new(Box::new(UdpDnsTransport::new(dns_server, 53)));

    match resolver.resolve_a(hostname) {
        Ok(addr) => IpcMessage::MethodReturn { id, result: Ok(ipc_str(&addr.to_string())) },
        Err(e) => IpcMessage::MethodReturn { id, result: Err(IpcError::new(ERROR_INTERNAL, e.to_string())) },
    }
}

...
fn ci_security(workspace_root: &Path) {
    build_uefi(workspace_root);

    eprintln!("CI security tests: booting QEMU and checking security subsystems");
    let result = security_tests::run_security_test_suite(workspace_root);
    for r in &result.results {
        let status = if r.passed { "PASS" } else { "FAIL" };
        eprintln!("  [{status}] {}: {}", r.name, r.detail);
    }
    if result.all_passed() {
        println!(
            "CI security tests PASSED: {}/{} tests passed",
            result.passed_count(),
            result.total_count()
        );
    } else {

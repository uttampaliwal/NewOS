...
pub fn create_process_pml4(
    ...
    let kernel_pml4_ptr = (physical_memory_offset + kernel_pml4_frame.start_address().as_u64()).as_ptr::<PageTable>();
    let new_pml4_ptr = (physical_memory_offset + new_frame.start_address().as_u64()).as_mut_ptr::<PageTable>();
...
pub unsafe fn set_user_accessible(
    virt_addr: VirtAddr,
    size: u64,
    ...
    let (pml4_frame, _) = Cr3::read();
    let pml4 = unsafe { &mut *((physical_mem_offset + pml4_frame.start_address().as_u64()).as_mut_ptr::<PageTable>()) };
    ...
    for addr in (start..end).step_by(4096) {
        ...
        let p3 = unsafe { &mut *((physical_mem_offset + p3_phys.as_u64()).as_mut_ptr::<PageTable>()) };
        ...
        let p2 = unsafe { &mut *((physical_mem_offset + p2_phys.as_u64()).as_mut_ptr::<PageTable>()) };
        ...
        let p1 = unsafe { &mut *((physical_mem_offset + p1_phys.as_u64()).as_mut_ptr::<PageTable>()) };

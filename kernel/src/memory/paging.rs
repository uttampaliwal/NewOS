...
pub unsafe fn init(
    ...
    let new_pml4_frame = frame_allocator
        .allocate_frame()
        .expect("Failed to allocate frame for new PML4");

    let uefi_pml4_ptr =
        (physical_memory_offset + uefi_pml4_frame.start_address().as_u64()).as_ptr::<PageTable>();
    let new_pml4_ptr = (physical_memory_offset + new_pml4_frame.start_address().as_u64())

...
impl Process {
        ...
        unsafe {
            ...
            for i in 0..header.program_header_count {
                if let Some(ph) = elf::parse_program_header(elf_data, header, i)? {
                    ...
                    crate::serial::println!("  segment {}: vaddr={:#x}, memsz={:#x}, filesz={:#x}", 
                        i, ph.virtual_address, ph.memory_size, ph.file_size);

                    let virt_start = VirtAddr::new(ph.virtual_address);
                    let mut extra_flags = PageTableFlags::empty();
                    if ph.flags & elf::PF_W != 0 {
                        extra_flags |= PageTableFlags::WRITABLE;
                    }
                    if ph.flags & elf::PF_X == 0 {
                        extra_flags |= PageTableFlags::NO_EXECUTE;
                    }
                    ...
                    while offset < ph.file_size {
                        ...
                        let chunk_phys = process_mapper
                            ...
                            .expect("user page not mapped during ELF load");
                        let dest_ptr =
                        ...
                        let src_ptr =
                            &elf_data[ph.file_offset as usize + offset as usize] as *const u8;
                    ...
                    while offset < ph.memory_size {
                        ...
                        let chunk_phys = process_mapper
                            .translate_addr(chunk_virt)
                            .expect("user page not mapped during ELF load BSS");
                        let dest_ptr =
                            (physical_memory_offset + chunk_phys.as_u64()).as_mut_ptr::<u8>();
    ...
    pub fn new(
        ...
        unsafe {
            ...
            use x86_64::structures::paging::Translate;
            let phys_code = process_mapper
                .translate_addr(code_start)
                .expect("failed to translate user code address");
    ...
    pub unsafe fn map_user_region(
        ...
        let pml4_ptr = (physical_memory_offset + self.pml4_frame.start_address().as_u64())
            .as_mut_ptr::<PageTable>();
        ...
        unsafe {
            ...
            for page in pages {
                use x86_64::structures::paging::Translate;
                if process_mapper.translate_addr(page.start_address()).is_some() {
                    continue;
                }
                ...
                // Ensure all levels have USER_ACCESSIBLE bit.
                // map_to only sets it on the last level if we don't handle it.
                // Actually OffsetPageTable::map_to should set it on newly created tables if the flags have USER.
                // But let's be explicit if needed or verify how map_to works.
            }
        }
    }
